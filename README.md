...
- 기본 카테고리: 사전 계산된 11개 카테고리 임베딩 사용
- 실행 경로: CPU/WASM
...
빠른 자동분류 실행
  ...
  ├─ 사진 전체 + 겹치는 4개 부분 화면 → 정규화된 image_embeds
  ├─ 각 카테고리의 긍정 프롬프트와 독립 비교
  ├─ 부분 화면·프롬프트 조합 중 가장 강한 증거를 채택
  └─ 기준을 넘은 모든 카테고리에 복수 배정
...
`사람`, `동물`, `음식`, `문서`, `영수증`, `풍경`, `건물`, `도시`, `실내`, `야간`, `스크린샷`

- 사람: 여러 사람, 셀카, 아기/어린이 포함
- 동물: 고양이, 강아지 포함
- 풍경: 바다, 산 포함
- 차량: 기본 카테고리에서 제거

한 사진은 여러 카테고리에 동시에 포함될 수 있습니다. 예를 들어 사람이 반려동물과 함께 있으면 `사람`과 `동물` 양쪽 결과에 나타납니다.

부정 프롬프트(`사람이 없는 사진` 등)는 CLIP이 부정어를 안정적으로 해석하지 못해 실제 사진에서 점수를 뒤집는 문제가 확인되어 기본 자동분류에서 제거했습니다.
...
- 현재 자동 정확도 검증은 공개 고양이/축구 이미지와 사람+동물 합성 사진의 기능 스모크 수준입니다. 100~200장 기준 정식 정확도 벤치마크는 아직 필요합니다.
- 실제 사용자 사진 2장 회귀 테스트에서 셀카+도시 풍경은 `사람·풍경·건물·도시·실내`, 도시 전경은 `풍경·도시·건물`로 복수 분류됐습니다.
